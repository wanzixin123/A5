...
## 3、任务5
        ○ SSE利用计算误方差和，来实现对不同K值的选取后，每个K值对应簇内的点到中心点的距离误差平方和，理论上SSE的值越
	小，代表聚类效果越好，通过数据测试，SSE的值会逐渐趋向一个最小值；同时增加迭代次数，观察聚类效果，发现到对应k
	值时，聚类效果变化不大，此时即为最佳k值
	○ ‘峰值’聚类、‘激光企业’聚类、‘电力用户用电’聚类、‘能源’聚类、‘省份GDP电量’聚类、‘电力用户缴费行为’聚类的k值
