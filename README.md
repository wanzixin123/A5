...
# 知识原理：
    ...
    3、电力客户行为分析
    	  k-means聚类分析：指定迭代次数终止训练，找到要聚类的簇的个数
	  聚类模型：按每天客户用电量画出用电曲线，再将其进行聚类，分出不同类用户，并将此模型持久	 化。之后再将每个类的用户显示出来，并将结果可视化
...
# 模型优化：
    ...
    3、任务5：将对象持久化
         先是写出一个模型，再将其持久化，之后再对其进行调用即可，用k-means聚类找到聚簇的簇数，之后再调用模型，将聚类曲线表示出来
